...
class StaffSalariesScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: itemsAsync.when(
          ...
          error: (error, _) => _StaffSalaryErrorView(
            ...
            if (items.isEmpty) {
              return ListView(
                ...
                children: [
                  ...
                  Icon(Icons.account_balance_wallet_outlined, size: 56, color: isDarkMode ? Colors.white24 : Colors.grey[300]),
                  const SizedBox(height: 12),
                  Text(
                    isSwahili ? 'Hakuna mishahara ya wafanyakazi' : 'No staff salaries found',
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    onTap: () => _showStaffSalarySheet(context, item, isDarkMode),
                    ...
                    trailing: PopupMenuButton<String>(
                      onSelected: (value) {
                        if (value == 'view') {
                          _showStaffSalarySheet(context, item, isDarkMode);
                        } else if (value == 'edit') {
  ...
  Future<void> _deleteItem(BuildContext context, WidgetRef ref, Map<String, dynamic> item) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        scrollable: true,
        title: Text(isSwahili ? 'Futa Mshahara' : 'Delete Staff Salary'),
        content: Text(
          isSwahili
              ? 'Je, unataka kufuta mshahara wa ${item['staff_name']}?'
              : 'Delete salary for ${item['staff_name']}?',
        ),
        actions: [
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isSwahili ? 'Mshahara umefutwa' : 'Staff salary deleted'),
...
void _showStaffSalarySheet(BuildContext context, Map<String, dynamic> item, bool isDarkMode) {
            ...
            children: [
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    Text(
                      item['staff_name']?.toString() ?? 'Staff Salary',
                      style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700),
                    ),
                    const SizedBox(height: 18),
                    _StaffSalaryDetailRow('Staff', item['staff_name']?.toString() ?? '-'),
                    _StaffSalaryDetailRow('Amount', 'TZS ${_formatMoney(item['amount'])}'),
                    _StaffSalaryDetailRow('Created', _formatDate(item['created_at']?.toString())),
                  ],
                ),
              ),
            ],
...
class _StaffSalaryFormSheet extends ConsumerStatefulWidget {
  ...
  const _StaffSalaryFormSheet({
    required this.refs,
    this.item,
  });

  @override
  ConsumerState<_StaffSalaryFormSheet> createState() => _StaffSalaryFormSheetState();
}
                  ...
                  Form(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          widget.item == null
                              ? (isSwahili ? 'Mshahara Mpya' : 'New Staff Salary')
                              : (isSwahili ? 'Hariri Mshahara' : 'Edit Staff Salary'),
                          textAlign: TextAlign.center,
                          style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700),
                        ),
...
class _StaffSalaryDetailRow extends StatelessWidget {
  final String label;
  final String value;

  const _StaffSalaryDetailRow(this.label, this.value);
...
class _StaffSalaryErrorView extends StatelessWidget {
  final String message;
  final bool isSwahili;
  final VoidCallback onRetry;

  const _StaffSalaryErrorView({
