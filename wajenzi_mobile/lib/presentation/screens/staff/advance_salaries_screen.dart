...
class AdvanceSalariesScreen extends ConsumerStatefulWidget {
  const AdvanceSalariesScreen({super.key});

  @override
  ConsumerState<AdvanceSalariesScreen> createState() =>
      _AdvanceSalariesScreenState();
}

class _AdvanceSalariesScreenState extends ConsumerState<AdvanceSalariesScreen> {
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              isSwahili
                                  ? 'Advance Salaries haipatikani kwenye live API kwa sasa.'
                                  : 'Advance Salaries is not available on the live API right now.',
                        ...
                        Text(
                          searchQuery.isNotEmpty || filterStatus != null
                              ? (isSwahili
                                    ...
                                    : 'No results found')
                              : (isSwahili
                                    ? 'Hakuna advance salaries'
                                    : 'No advance salaries found'),
                          textAlign: TextAlign.center,
                          style: TextStyle(
                            fontSize: 16,
  ...
  Future<void> _deleteItem(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        title: Text(
          isSwahili ? 'Futa Advance Salary' : 'Delete Advance Salary',
          style: TextStyle(color: isDarkMode ? Colors.white : null),
        ),
        content: Text(
          isSwahili
              ? 'Una uhakika unataka kufuta advance salary ya "${item['staff_name']}"?'
              : 'Are you sure you want to delete advance salary for "${item['staff_name']}"?',
    ...
    try {
      await ref
          ...
          .delete('/advance-salaries/${item['id']}');
      ref.invalidate(_advanceSalariesProvider);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              isSwahili ? 'Advance salary imefutwa' : 'Advance salary deleted',
            ),
...
class _AdvanceSalaryCard extends StatelessWidget {
...
class _AdvanceSalaryFormSheet extends ConsumerStatefulWidget {
  ...
  const _AdvanceSalaryFormSheet({required this.refs, this.item});

  @override
  ConsumerState<_AdvanceSalaryFormSheet> createState() =>
      _AdvanceSalaryFormSheetState();
}

class _AdvanceSalaryFormSheetState
    extends ConsumerState<_AdvanceSalaryFormSheet> {
