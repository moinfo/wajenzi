...
class ServiceTypesScreen extends ConsumerWidget {
  ...
  Future<void> _openForm(
    ...
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (_) => FractionallySizedBox(
        heightFactor: 0.52,
        child: _ServiceTypeFormSheet(item: item),
      ),
    );
    if (result == true) {
  ...
  Future<void> _deleteItem(
    ...
    try {
      await ref.read(apiClientProvider).delete('/service-types/${item['id']}');
      ref.invalidate(_serviceTypesProvider);
...
class _ServiceTypeFormSheet extends ConsumerStatefulWidget {
...
class _ServiceTypeFormSheetState extends ConsumerState<_ServiceTypeFormSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_isEdit) {
        await api.put('/service-types/${widget.item!['id']}', data: payload);
      } else {
        await api.post('/service-types', data: payload);
      }
      if (!mounted) return;
