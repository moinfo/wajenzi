...
class RolesScreen extends ConsumerWidget {
  ...
  Future<void> _openForm(
    ...
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (_) => FractionallySizedBox(
        heightFactor: 0.52,
        child: _RoleFormSheet(item: item),
      ),
    );
    if (result == true) {
  ...
  Future<void> _deleteItem(
    ...
    try {
      await ref.read(apiClientProvider).delete('/roles/${item['id']}');
      ref.invalidate(_rolesProvider);
...
class _RoleFormSheet extends ConsumerStatefulWidget {
  ...
  const _RoleFormSheet({this.item});
...
class _RoleFormSheetState extends ConsumerState<_RoleFormSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_isEdit) {
        await api.put('/roles/${widget.item!['id']}', data: payload);
      } else {
        await api.post('/roles', data: payload);
      }
      if (!mounted) return;
