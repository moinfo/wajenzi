...
final _statutoryPaymentReportYearProvider = StateProvider.autoDispose<int>(
  (ref) => DateTime.now().year,
);

final _statutoryPaymentReportProvider =
    FutureProvider.autoDispose<Map<String, dynamic>>((ref) async {
  final api = ref.watch(apiClientProvider);
  final year = ref.watch(_statutoryPaymentReportYearProvider);
  final response = await api.get(
    '/reports/statutory-payment-report',
...
class StatutoryPaymentReportScreen extends ConsumerWidget {
  const StatutoryPaymentReportScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final isSwahili = ref.watch(isSwahiliProvider);
    final isDarkMode = ref.watch(isDarkModeProvider);
    final selectedYear = ref.watch(_statutoryPaymentReportYearProvider);
    final reportAsync = ref.watch(_statutoryPaymentReportProvider);

    return Scaffold(
      appBar: AppBar(
        title: Text(
          isSwahili
              ? 'Ripoti ya Malipo ya Kisheria'
              : 'Statutory Payment Report',
        ),
      ),
      body: RefreshIndicator(
        onRefresh: () async => ref.invalidate(_statutoryPaymentReportProvider),
        child: reportAsync.when(
          loading: () => const Center(child: CircularProgressIndicator()),
          error: (error, _) => _ReportErrorView(
            isSwahili: isSwahili,
            message: vatErrorMessage(error, isSwahili: isSwahili),
            onRetry: () => ref.invalidate(_statutoryPaymentReportProvider),
          ),
          data: (report) {
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<int>(
                          ...
                          onChanged: (value) {
                            ...
                            ref.read(_statutoryPaymentReportYearProvider.notifier).state =
                                value;
                          },
                        ),
                      ],
                      ...
                      children: [
                        ...
                        ...footer.map(
                          (item) => Padding(
                            ...
                            child: _SummaryRow(
                              label: item['sub_category_name']?.toString() ?? '-',
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 12),
                            ...((row['sub_categories'] as List? ?? const [])
                                .whereType<Map>()
                                .map((item) => Map<String, dynamic>.from(item))
                                .map(
                                  (item) => Padding(
