...
class BoqItemCategoriesScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isSwahili ? 'BOQ Item Categories' : 'BOQ Item Categories'),
        actions: [
          IconButton(
      ...
      body: RefreshIndicator(
        ...
        child: categoriesAsync.when(
          ...
          error: (error, _) => _BoqCategoryErrorView(
            ...
            if (categories.isEmpty) {
              return ListView(
                ...
                children: [
                  ...
                  Text(
                    isSwahili ? 'Hakuna BOQ item categories' : 'No BOQ item categories found',
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(
                      ...
                      child: Icon(isChild ? Icons.subdirectory_arrow_right : Icons.folder_outlined, color: isChild ? AppColors.info : AppColors.primary),
                    ),
                    title: Text(
                    ...
                    trailing: PopupMenuButton<String>(
                      onSelected: (value) {
                        if (value == 'view') {
                          _showDetails(context, category, isDarkMode);
                        } else if (value == 'edit') {
                    ...
                    onTap: () => _showDetails(context, category, isDarkMode),
                  ),
                );
              },
            );
  ...
  Future<void> _openForm(BuildContext context, WidgetRef ref, {Map<String, dynamic>? category}) async {
    ...
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (_) => FractionallySizedBox(
        heightFactor: 0.86,
        child: _BoqItemCategoryFormSheet(refs: refs, category: category),
      ),
    );
    if (result == true) ref.invalidate(_boqItemCategoriesProvider);
  }

  Future<void> _deleteCategory(BuildContext context, WidgetRef ref, Map<String, dynamic> category) async {
    final isSwahili = ref.read(isSwahiliProvider);
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        scrollable: true,
        title: Text(isSwahili ? 'Futa Category' : 'Delete Category'),
        content: Text(isSwahili ? 'Je, unataka kufuta ${category['name']}?' : 'Delete ${category['name']}?'),
        actions: [
    ...
    try {
      await ref.read(apiClientProvider).delete('/boq-item-categories/${category['id']}');
      ref.invalidate(_boqItemCategoriesProvider);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isSwahili ? 'Category imefutwa' : 'Category deleted'),
  ...
  void _showDetails(BuildContext context, Map<String, dynamic> category, bool isDarkMode) {
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      ...
                      _detailLine('Parent', category['parent_name'] ?? 'Top Level'),
                      _detailLine('Description', category['description']),
                      _detailLine('Sort Order', category['sort_order']),
                      _detailLine('Status', (category['is_active'] == true) ? 'Active' : 'Inactive'),
                      _detailLine('Children', category['children_count']),
                    ],
                  ),
                ),
              ],
...
class _BoqItemCategoryFormSheet extends ConsumerStatefulWidget {
  ...
  const _BoqItemCategoryFormSheet({
    required this.refs,
    this.category,
  });

  @override
  ConsumerState<_BoqItemCategoryFormSheet> createState() => _BoqItemCategoryFormSheetState();
}

class _BoqItemCategoryFormSheetState extends ConsumerState<_BoqItemCategoryFormSheet> {
  final _formKey = GlobalKey<FormState>();
  late final TextEditingController _nameController = TextEditingController(text: widget.category?['name']?.toString() ?? '');
  late final TextEditingController _descriptionController = TextEditingController(text: widget.category?['description']?.toString() ?? '');
  late final TextEditingController _sortOrderController = TextEditingController(text: widget.category?['sort_order']?.toString() ?? '0');
  int? _parentId;
  bool _isActive = true;
  bool _saving = false;
  ...
  void initState() {
    ...
    _isActive = widget.category?['is_active'] == null ? true : widget.category!['is_active'] == true;
  }
                ...
                children: [
                  Form(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          widget.category == null ? 'New BOQ Item Category' : 'Edit BOQ Item Category',
                          textAlign: TextAlign.center,
                          style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700),
                        ),
                        const SizedBox(height: 20),
                        _input(_nameController, isSwahili ? 'Category Name *' : 'Category Name *', isDarkMode),
                        const SizedBox(height: 12),
                        _dropdown(
                          isDarkMode: isDarkMode,
                          label: isSwahili ? 'Parent Category' : 'Parent Category',
                          items: [
                        ...
                        ElevatedButton(
                          ...
                          child: _saving
                              ...
                              : Text(widget.category == null ? (isSwahili ? 'Hifadhi' : 'Save') : (isSwahili ? 'Sasisha' : 'Update')),
                        ),
                      ],
                    ),
                  ),
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (widget.category == null) {
        ...
        await api.put('/boq-item-categories/${widget.category!['id']}', data: data);
      }
...
List<Map<String, dynamic>> _sortBoqCategories(List<Map<String, dynamic>> categories) {
  final parents = categories.where((category) => category['parent_id'] == null).toList()
    ..sort((a, b) {
  ...
  for (final category in categories.where((item) => item['parent_id'] != null)) {
