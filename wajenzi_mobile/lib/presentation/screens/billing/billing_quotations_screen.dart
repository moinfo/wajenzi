...
class BillingQuotationsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isSwahili ? 'Quotations' : 'Billing Quotations'),
        actions: [
          IconButton(
            ...
            if (items.isEmpty) {
              return ListView(
                ...
                children: [
                  ...
                  Text(
                    isSwahili
                        ? 'Bonyeza alama ya kuongeza kuunda quotation mpya.'
                        : 'Tap the add button to create a quotation.',
                    textAlign: TextAlign.center,
...
Future<void> _openQuotationForm(
  ...
  if (quotation != null && quotationId > 0 && !hasItems) {
    initialQuotation = await ref.read(_quotationDetailProvider(quotationId).future);
  }
...
Future<void> _deleteQuotation(
  ...
  final confirmed = await showDialog<bool>(
    ...
    builder: (dialogContext) => AlertDialog(
      scrollable: true,
      title: Text(isSwahili ? 'Futa Quotation' : 'Delete Quotation'),
      content: Text(
        isSwahili
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(
            isSwahili ? 'Quotation imefutwa' : 'Quotation deleted',
          ),
        ),
      );
    }
            ...
            child: SafeArea(
              ...
              child: detailAsync.when(
                ...
                error: (error, _) => _QuotationErrorView(
                  isSwahili: isSwahili,
                  message: vatErrorMessage(error, isSwahili: isSwahili),
                  onRetry: () => ref.invalidate(_quotationDetailProvider(id)),
                ),
                data: (detail) {
                  final client =
                  ...
                  children: [
                    Text(
                      widget.quotation == null
                          ? (isSwahili ? 'Quotation Mpya' : 'New Quotation')
                          : (isSwahili ? 'Hariri Quotation' : 'Edit Quotation'),
                      textAlign: TextAlign.center,
                      style: const TextStyle(
                        fontSize: 20,
                        fontWeight: FontWeight.w700,
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: Text(
                            isSwahili ? 'Vitu vya quotation' : 'Quotation Items',
                            style: const TextStyle(
                              fontSize: 16,
                              fontWeight: FontWeight.w700,
                            ),
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(
            quotationId > 0
                ? (isSwahili ? 'Quotation imesasishwa' : 'Quotation updated')
                : (isSwahili ? 'Quotation imehifadhiwa' : 'Quotation created'),
          ),
        ),
      );
    } catch (error) {
