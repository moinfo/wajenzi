// Smoke test: log in against the local API and open each Site Paylog screen,
// asserting it renders. Run on a booted device/emulator with:
//   flutter test integration_test/paylog_smoke_test.dart -d <deviceId> \
...
void main() {
  ...
  testWidgets('login and browse site paylog screens', (tester) async {
    ...
    await visit('/site-paylog', 'Daily Payments');
    await visit('/site-paylog/requests', 'Payment Requests');
    await visit('/site-paylog/channels', 'Payment Channels');
    await visit('/site-paylog/daily-report', 'Daily Payment Report');
    await visit('/site-paylog/monthly-report', 'Monthly Payment Report');
  });
}
