// Smoke test: log in against the local API and open each Performance/KPI
// screen (incl. the new Templates + Supervisor Assignments), asserting each
// renders. Run on a booted device/emulator with:
//   flutter test integration_test/performance_smoke_test.dart -d <deviceId> \
...
void main() {
  ...
  testWidgets('login and browse performance/KPI screens', (tester) async {
    ...
    await visit('/performance', 'My Performance');
    await visit('/performance/templates', 'KPI Templates');
    await visit('/performance/supervisor-assignments', 'Supervisor Assignments');
  });
}
