...
class SupplierReceiving extends Model
{
    ...
    {
        $start_date = '2020-01-01';
        return SupplierReceiving::select([DB::raw("SUM(supplier_receivings.amount) as total_amount")])->join('suppliers','suppliers.id', '=' , 'supplier_receivings.supplier_id')->join('systems','systems.id', '=' , 'suppliers.system_id')->WhereBetween('supplier_receivings.date',[$start_date,$end_date])->Where('suppliers.system_id',$system_id)->groupBy('suppliers.system_id')->get()->first()['total_amount'] ?? 0;
    }
    ...
    static function getTotalSupplierReceivingPerDay($date){
        return \App\Models\SupplierReceiving::Where('date',$date)->select([DB::raw("SUM(amount) as total_amount")])->groupBy('date')->get()->first()['total_amount'] ?? 0;
    }

    static function getTotalSupplierReceivingToAllSuppliers($start_date, $end_date){
        return \App\Models\SupplierReceiving::whereBetween('date', [$start_date, $end_date])->select([DB::raw("SUM(amount) as total_amount")])->get()->first()['total_amount'] ?? 0;
    }
}
