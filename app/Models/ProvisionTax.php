...
class ProvisionTax extends Model
{
   ...
   public static function Profit_From_Operating_Activities_After_Provision($start_date,$end_date){
        return \App\Models\ProvisionTax::Where('date','>=',$start_date)->Where('date','<=',$end_date)->select([DB::raw("SUM(amount) as total_amount")])->groupBy('date')->get()->first()['total_amount'] ?? 0;
    }
