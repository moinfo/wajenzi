...
class ProjectInvoiceController extends Controller
{
    public function index(Request $request) {
        //handle crud operations
        if($this->handleCrud($request, 'ProjectInvoice')) {
            return back();
        }

        $invoices = ProjectInvoice::with(['project', 'payments'])
        ...
        return view('pages.projects.invoices_index')->with($data);
    }
    ...
    public function markAsOverdue() {
        $overdue_invoices = ProjectInvoice::where('due_date', '<', now())
        ...
        foreach($overdue_invoices as $invoice) {
            ...
            // Send notifications
            $invoice->project->manager->notify(new InvoiceOverdue($invoice));
            $invoice->project->client->notify(new InvoiceOverdue($invoice));
        }

        return response()->json([
            'success' => true,
            'updated_count' => count($overdue_invoices)
        ]);
    }
}
