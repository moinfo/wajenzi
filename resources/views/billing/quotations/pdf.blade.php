...
<body>
                ...
                <table style="margin-left: auto;">
                    @if($quotation->reference_number)
                        <tr>
                            ...
                            <td class="info-value">{{ $quotation->reference_number }}</td>
                        </tr>
                    @endif
                    <tr>
                        <td class="info-label">Issue Date:</td>
                        <td class="info-value">{{ $quotation->issue_date->format('d/m/Y') }}</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="info-value">{{ ucwords(str_replace('_', ' ', $quotation->payment_terms)) }}</td>
                    </tr>
                    @if($quotation->po_number)
                        <tr>
                            <td class="info-label">PO Number:</td>
                            <td class="info-value">{{ $quotation->po_number }}</td>
                        </tr>
                    @endif
                    @if($quotation->sales_person)
                        <tr>
                            <td class="info-label">Sales Person:</td>
                            <td class="info-value">{{ $quotation->sales_person }}</td>
                        </tr>
                    @endif
                </table>
            </td>
    ...
    <table class="two-boxes">
        <tr>
            <td style="padding-right: 10px; width: 42%;">
                <div class="bill-to-box">
                    ...
                    @if($quotation->client->phone_number)
                        <strong>Phone:</strong> {{ $quotation->client->phone_number }}<br>
                    @endif
            ...
            <td style="padding-left: 10px; vertical-align: top;">
                <span class="invoice-for-label">QUOTATION FOR :</span>
                @if($quotation->title)
                    <span class="invoice-for-title">{{ $quotation->title }}</span>
                @endif
    ...
    <table class="totals-table">
        <tr>
            ...
            <td style="text-align:right;">{{ $quotation->currency_code }} {{ number_format($quotation->subtotal_amount, 2) }}</td>
        </tr>
        @if($quotation->discount_amount > 0)
            <tr>
                <td><strong>Discount:</strong></td>
                <td style="text-align:right; color: #28a745;">-{{ $quotation->currency_code }} {{ number_format($quotation->discount_amount, 2) }}</td>
            </tr>
        @endif
        @if($quotation->tax_amount > 0)
            <tr>
                <td><strong>Tax:</strong></td>
                <td style="text-align:right;">{{ $quotation->currency_code }} {{ number_format($quotation->tax_amount, 2) }}</td>
            </tr>
        @endif
        @if($quotation->shipping_amount > 0)
            <tr>
                <td><strong>Shipping:</strong></td>
                <td style="text-align:right;">{{ $quotation->currency_code }} {{ number_format($quotation->shipping_amount, 2) }}</td>
            </tr>
        @endif
        <tr class="total-row">
            <td><strong>TOTAL:</strong></td>
            <td style="text-align:right;"><strong>{{ $quotation->currency_code }} {{ number_format($quotation->total_amount, 2) }}</strong></td>
        </tr>
    </table>
    ...
    @if($quotation->valid_until_date)
        <div class="payment-instructions">
            ...
            This quotation is valid until <strong>{{ $quotation->valid_until_date->format('d/m/Y') }}</strong>.<br>
            @if($quotation->valid_until_date->isPast() && $quotation->status != 'accepted')
                <span style="color: #dc3545;"><strong>This quotation has expired.</strong></span>
            @else
                Please confirm your acceptance before the expiry date to secure these rates.
            @endif
        </div>
    @endif
        ...
        <table style="width: 100%; font-size: 10px;">
            <tr>
                ...
                <td width="50%">
                    ...
                    <strong>Reference:</strong> {{ $quotation->document_number }}
                </td>
            </tr>
        </table>
    </div>
