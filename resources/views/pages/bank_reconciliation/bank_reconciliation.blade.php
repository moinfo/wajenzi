...
@section('content')
    ...
    $base_route = 'bank_reconciliations/'.$route_id.'/'.$route_document_type_id;
            ...
            <div>
                <div class="block block-themed">
                    <div class="block-header bg-gd-lake">
                        <h3 class="block-title">{{$bank_reconciliation->supplier->name ?? null }}</h3>
                    </div>
                    <div class="block-content">
                        <form method="post" action="{{route('hr_settings_approvals')}}" enctype="multipart/form-data">
                            ...
                            <table class="table table-bordered table-striped table-vcenter">
                                <tbody>
                                <tr>
                                    <th width="30%">EFD Name</th>
                                    <td>{{ $bank_reconciliation->efd->name ?? null }}</td>
                                </tr>
                                <tr>
                                    <th width="30%">REFERENCE</th>
                                    <td>{{$bank_reconciliation->reference ?? null}}</td>
                                </tr>
                                <tr>
                                    <th width="30%">Payment Type</th>
                                    <td>{{$bank_reconciliation->payment_type ?? null}}</td>
                                </tr>
                                <tr>
                                    <th width="30%">Description</th>
                                    <td>{{$bank_reconciliation->description ?? null}}</td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    @if($bank_reconciliation->status == 'PENDING')
                                        <td width="70%" class="bold text-capitalize"
                                            ...
                                            <div
                                                class="badge badge-warning badge-pill">{{ $bank_reconciliation->status}}</div>
                                        </td>
                                    @elseif($bank_reconciliation->status == 'APPROVED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-primary badge-pill">{{ $bank_reconciliation->status}}</div>
                                        </td>
                                    @elseif($bank_reconciliation->status == 'REJECTED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-danger badge-pill">{{ $bank_reconciliation->status}}</div>
                                        </td>
                                    ...
                                    @elseif($bank_reconciliation->status == 'COMPLETED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-success badge-pill">{{ $bank_reconciliation->status}}</div>
                                        </td>
                                    @else
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-secondary badge-pill">{{ $bank_reconciliation->status}}</div>
                                        </td>
                                    @endif
                                </tr>
                                </tbody>
                            ...
                            <div class="row">
                                ...
                                @if($nextApproval)
                                    @if($rejected)
                                        <div class="col-md-9">
                                            <span class='pull-right'>This Bank Reconciliation was rejected <i class='text-light'>Comment: {{$rejected->comments}}</i></span>
                                        </div>
                                        <div class="col-md-3">
                                        </div>
                                    @else
                                        ...
                                        @else
                                            <div class="col-md-12">
                                                ...
                                                <input type="hidden" name="link" id="link" value="bank_reconciliations/{{$document_id}}/12">
                                                ...
                                                <input type="hidden" name="document_type_id" id="document_type_id" value="12">
                                                ...
                                                <div class="form-group">
                                                    <div class="btn-group pull-right">
                                                        <button type="submit" class="btn btn-alt-primary btn-sm" name="approveItem" value="BankReconciliation">Approve now</button>
                                                        <button type="submit" class="btn btn-alt-danger btn-sm" name="rejectItem" value="BankReconciliation">Reject</button>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                ...
                                @elseif($approvalCompleted)
                                    <div class="col-md-9">
                                        <span class='text-primary'><i class='fa fa-check '>&nbsp;&nbsp;&nbsp;</i> Bank Reconciliation Approved</span>
                                    </div>
                                    <div class="col-md-3">
