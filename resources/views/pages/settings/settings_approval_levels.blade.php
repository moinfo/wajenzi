        ...
        <div class="content">
            <div class="content-heading">Settings
                <div class="float-right">
                    @if(\App\Models\UsersPermission::isUserAllowed(Auth::user()->id,"CRUD","Add Approval Level"))
                        <button type="button" onclick="loadFormModal('settings_approval_level_form', {className: 'ApprovalLevel'}, 'Create New Approval Level', 'modal-md');" class="btn btn-rounded btn-outline-primary min-width-125 mb-10">
                            <i class="si si-plus">&nbsp;</i>New Approval Level</button> @endif
...
{{--                                <th>User Group Keyword</th>--}}
                            ...
                            @foreach ($approval_levels as $key => $value)
                                <tr>
                                    ...
                                    <td>{{ $value->approvalDocumentType->name }}</td>
                                    <td>{{ $value->order }}</td>
...
{{--                                    <td>{{ $value->userGroup->keyword}}</td>--}}
                                    ...
                                    <td class="text-center" >
                                        <div class="btn-group">
                                            @if(\App\Models\UsersPermission::isUserAllowed(Auth::user()->id,"CRUD","Edit Approval Level"))
                                            ...
                                            @if(\App\Models\UsersPermission::isUserAllowed(Auth::user()->id,"CRUD","Delete Approval Level"))
