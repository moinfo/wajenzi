...
@section('content')
    ...
    $base_route = 'supplier_receiving/'.$route_id.'/'.$route_document_type_id;
    ...
    <div class="container-fluid">
        <div class="content">
            <div class="content-heading">SupplierReceiving
            ...
            <div>
                <div class="block block-themed">
                    <div class="block-header bg-gd-lake">
                        <h3 class="block-title">{{ $supplier_receiving->supplier->name ?? null }}</h3>
                    </div>
                        ...
                        <form method="post" action="{{route('hr_settings_approvals')}}" enctype="multipart/form-data">
                            ...
                            <table class="table table-bordered table-striped table-vcenter">
                                ...
                                <tr>
                                    ...
                                    <td>{{$supplier_receiving->description ?? null}}</td>
                                </tr>
                                <tr>
                                    <th>Total Amount</th>
                                    <td>{{ number_format($supplier_receiving->amount, 2)}}</td>
                                </tr>
                                <tr>
                                    <th>Date</th>
                                ...
                                <tr>
                                    ...
                                    <td width="70%" class="bold">
                                        ...
                                        <a href="{{ url($supplier_receiving->file) }}" target="_blank">View</a>
                                            @endif
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    @if($supplier_receiving->status == 'PENDING')
                                        <td width="70%" class="bold text-capitalize"
                                            ...
                                            <div
                                                class="badge badge-warning badge-pill">{{ $supplier_receiving->status}}</div>
                                        </td>
                                    @elseif($supplier_receiving->status == 'APPROVED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-primary badge-pill">{{ $supplier_receiving->status}}</div>
                                        </td>
                                    @elseif($supplier_receiving->status == 'REJECTED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-danger badge-pill">{{ $supplier_receiving->status}}</div>
                                        </td>
                                    ...
                                    @elseif($supplier_receiving->status == 'COMPLETED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-success badge-pill">{{ $supplier_receiving->status}}</div>
                                        </td>
                                    @else
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-secondary badge-pill">{{ $supplier_receiving->status}}</div>
                                        </td>
                                    @endif
                                </tr>
                                </tbody>
                            ...
                            <div class="row">
                                ...
                                @if($nextApproval)
                                    @if($rejected)
                                        <div class="col-md-9">
                                            <span class='pull-right'>This SupplierReceivings was rejected <i class='text-light'>Comment: {{$rejected->comments}}</i></span>
                                        </div>
                                        <div class="col-md-3">
                                        </div>
                                    @else
                                        ...
                                        @else
                                            <div class="col-md-12">
                                                ...
                                                <input type="hidden" name="link" id="link" value="supplier_receiving/{{$document_id}}/10">
                                                ...
                                                <input type="hidden" name="document_type_id" id="document_type_id" value="10">
                                                ...
                                                <div class="form-group">
                                                    <div class="btn-group pull-right">
                                                        <button type="submit" class="btn btn-alt-primary btn-sm" name="approveItem" value="SupplierReceiving">Approve now</button>
                                                        <button type="submit" class="btn btn-alt-danger btn-sm" name="rejectItem" value="SupplierReceiving">Reject</button>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                ...
                                @elseif($approvalCompleted)
                                    <div class="col-md-9">
                                        <span class='text-primary'><i class='fa fa-check '>&nbsp;&nbsp;&nbsp;</i> SupplierReceivings Approved</span>
                                    </div>
                                    <div class="col-md-3">
