            ...
            <div>
                <div class="block block-themed">
                    <div class="block-header bg-gd-dusk">
                        <h3 class="block-title">Annually Expenses Summary Report</h3>
                    </div>
                            ...
                            <table class="table table-bordered table-striped table-vcenter js-dataTable-full" data-ordering="false">
                                ...
                                @foreach ($period as $dt)
                                    ...
                                    <tr>
                                        ...
                                        @foreach($expenses_categories as $expenses_category)
                                            @php
                                                $expenses_category_id = $expenses_category->id;
                                                    $charges = \App\Models\Expense::getTotalExpensesGroupByExpensesCategory($start_date,$end_date,$expenses_category_id);
                                                   $total_charges += $charges;
                                            @endphp
                                            <td class="text-right">{{number_format($charges)}}</td>
                                        @endforeach
                                        @php
                                            $all_charges = \App\Models\Expense::getTotalExpensesGroupByExpensesCategory($start_date,$end_date,null);
                                        @endphp
                                ...
                                <tr>
                                    ...
                                    @foreach($expenses_categories as $expenses_category)
                                        @php
                                            $expenses_category_id = $expenses_category->id;
                                                $charges = \App\Models\Expense::getTotalExpensesGroupByExpensesCategory($start_date_year,$end_date_year,$expenses_category_id);
                                        @endphp
                                        <td class="text-right">{{number_format($charges)}}</td>
                                    @endforeach
                                    @php
                                        $total_all_charges = \App\Models\Expense::getTotalExpensesGroupByExpensesCategory($start_date_year,$end_date_year,null);
                                    @endphp
