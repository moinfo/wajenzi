...
//                                            $id = $supervisor->id;
                                           $expense = \App\Models\Expense::Where('status','APPROVED')->Where('date',$date)->select([DB::raw("SUM(amount) as total_amount")])->groupBy('date')->get()->first();
...
{{--                                    @foreach ($supervisors as $supervisor)--}}
                                        <?php
                                        $total_expense_by_supervisor = \App\Models\Expense::Where('status','APPROVED')->whereBetween('date', [$start_date, $end_date])->select([DB::raw("SUM(amount) as total_amount")])->groupBy('supervisor_id')->get()->first();
                                        ?>
