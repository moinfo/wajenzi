                        ...
                        @elseif($system == 1)
                            ...
                            @foreach($systems as $system)
                                ...
                                @foreach($suppliers as $supplier)
                                    @php
                                        $supplier_id = $supplier->id;
                                        $amount = \App\Models\BankReconciliation::select(\Illuminate\Support\Facades\DB::raw("SUM(bank_reconciliations.debit) as total_debit"))->join('suppliers','suppliers.id','=','bank_reconciliations.supplier_id')->where('suppliers.supplier_depend_on_system','BONGE')->whereBetween('bank_reconciliations.date',[$start_date,$end_date])->get()->first()->total_debit;
