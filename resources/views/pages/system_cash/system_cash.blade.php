...
@section('content')
    ...
    $base_route = 'system_cash/'.$route_id.'/'.$route_document_type_id;
    ...
    <div class="container-fluid">
        <div class="content">
            <div class="content-heading">SystemCash
            ...
            <div>
                <div class="block block-themed">
                    <div class="block-header bg-gd-lake">
                        <h3 class="block-title">{{$system_cash->system->name ?? null }}</h3>
                    </div>
                    <div class="block-content">
                        <form method="post" action="{{route('hr_settings_approvals')}}" enctype="multipart/form-data">
                            ...
                            <table class="table table-bordered table-striped table-vcenter">
                                ...
                                <tr>
                                    <th>Total Amount</th>
                                    <td>{{ number_format($system_cash->amount, 2)}}</td>
                                </tr>
                                <tr>
                                    <th>Date</th>
                                ...
                                <tr>
                                    ...
                                    @if($system_cash->status == 'PENDING')
                                        <td width="70%" class="bold text-capitalize"
                                            ...
                                            <div
                                                class="badge badge-warning badge-pill">{{ $system_cash->status}}</div>
                                        </td>
                                    @elseif($system_cash->status == 'APPROVED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-primary badge-pill">{{ $system_cash->status}}</div>
                                        </td>
                                    @elseif($system_cash->status == 'REJECTED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-danger badge-pill">{{ $system_cash->status}}</div>
                                        </td>
                                    ...
                                    @elseif($system_cash->status == 'COMPLETED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-success badge-pill">{{ $system_cash->status}}</div>
                                        </td>
                                    @else
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-secondary badge-pill">{{ $system_cash->status}}</div>
                                        </td>
                                    @endif
                                </tr>
                                </tbody>
                            ...
                            <div class="row">
                                ...
                                @if($nextApproval)
                                    @if($rejected)
                                        <div class="col-md-9">
                                            <span class='pull-right'>This SystemCashs was rejected <i class='text-light'>Comment: {{$rejected->comments}}</i></span>
                                        </div>
                                        <div class="col-md-3">
                                        </div>
                                    @else
                                        ...
                                        @else
                                            <div class="col-md-12">
                                                ...
                                                <input type="hidden" name="link" id="link" value="system_cash/{{$document_id}}/14">
                                                ...
                                                <input type="hidden" name="document_type_id" id="document_type_id" value="14">
                                                ...
                                                <div class="form-group">
                                                    <div class="btn-group pull-right">
                                                        <button type="submit" class="btn btn-alt-primary btn-sm" name="approveItem" value="SystemCash">Approve now</button>
                                                        <button type="submit" class="btn btn-alt-danger btn-sm" name="rejectItem" value="SystemCash">Reject</button>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                ...
                                @elseif($approvalCompleted)
                                    <div class="col-md-9">
                                        <span class='text-primary'><i class='fa fa-check '>&nbsp;&nbsp;&nbsp;</i> SystemCashs Approved</span>
                                    </div>
                                    <div class="col-md-3">
