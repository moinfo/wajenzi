...
@section('content')
    ...
    $base_route = 'system_capital/'.$route_id.'/'.$route_document_type_id;
            ...
            <div>
                <div class="block block-themed">
                    <div class="block-header bg-gd-lake">
                        <h3 class="block-title">{{$system_capital->system->name ?? null }}</h3>
                    </div>
                    <div class="block-content">
                        <form method="post" action="{{route('hr_settings_approvals')}}" enctype="multipart/form-data">
                            ...
                            <table class="table table-bordered table-striped table-vcenter">
                                ...
                                <tr>
                                    <th>Total Amount</th>
                                    <td>{{ number_format($system_capital->amount, 2)}}</td>
                                </tr>
                                <tr>
                                    <th>Date</th>
                                ...
                                <tr>
                                    ...
                                    @if($system_capital->status == 'PENDING')
                                        <td width="70%" class="bold text-capitalize"
                                            ...
                                            <div
                                                class="badge badge-warning badge-pill">{{ $system_capital->status}}</div>
                                        </td>
                                    @elseif($system_capital->status == 'APPROVED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-primary badge-pill">{{ $system_capital->status}}</div>
                                        </td>
                                    @elseif($system_capital->status == 'REJECTED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-danger badge-pill">{{ $system_capital->status}}</div>
                                        </td>
                                    ...
                                    @elseif($system_capital->status == 'COMPLETED')
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-success badge-pill">{{ $system_capital->status}}</div>
                                        </td>
                                    @else
                                        <td width="70%" class="bold text-capitalize"
                                            style="font-size: 16px!important">
                                            <div
                                                class="badge badge-secondary badge-pill">{{ $system_capital->status}}</div>
                                        </td>
                                    @endif
                                </tr>
                                </tbody>
                            ...
                            <div class="row">
                                ...
                                @if($nextApproval)
                                    @if($rejected)
                                        <div class="col-md-9">
                                            <span class='pull-right'>This System Receiving was rejected <i class='text-light'>Comment: {{$rejected->comments}}</i></span>
                                        </div>
                                        <div class="col-md-3">
                                        </div>
                                    @else
                                        ...
                                        @else
                                            <div class="col-md-12">
                                                ...
                                                <input type="hidden" name="link" id="link" value="system_capital/{{$document_id}}/16">
                                                ...
                                                <input type="hidden" name="document_type_id" id="document_type_id" value="16">
                                                ...
                                                <div class="form-group">
                                                    <div class="btn-group pull-right">
                                                        <button type="submit" class="btn btn-alt-primary btn-sm" name="approveItem" value="SystemCapital">Approve now</button>
                                                        <button type="submit" class="btn btn-alt-danger btn-sm" name="rejectItem" value="SystemCapital">Reject</button>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                ...
                                @elseif($approvalCompleted)
                                    <div class="col-md-9">
                                        <span class='text-primary'><i class='fa fa-check '>&nbsp;&nbsp;&nbsp;</i> System Receiving Approved</span>
                                    </div>
                                    <div class="col-md-3">
