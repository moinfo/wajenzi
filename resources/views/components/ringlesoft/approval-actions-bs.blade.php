{{-- Tracked override of ringlesoft/laravel-process-approval bootstrap approval-actions component.
     Identical to the vendor view, except each approval now renders its comment as visible text
     (the vendor view only exposes the comment via a hover popover/tooltip).
     Rendered by App\View\Components\ApprovalActions, registered in AppServiceProvider. --}}
                                ...
                                <td>
                                    <div class="d-flex flex-column justify-content-center align-items-center">
                                        @if($currentApproval = $item['approval'])
                                            ...
                                            @if(filled($currentApproval->comment))
                                                <div class="small fst-italic text-body mt-1 px-1"
                                                     style="white-space: normal; word-break: break-word; max-width: 240px; text-align: center;">
                                                    <i class="fa fa-comment-dots text-muted"></i>
                                                    &ldquo;{{ $currentApproval->comment }}&rdquo;
                                                </div>
                                            @endif
                                        @endif
                                    </div>
