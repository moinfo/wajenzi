    ...
    <form method="post"  enctype="multipart/form-data" autocomplete="off">
        ...
        <div class="form-group">
            ...
            <select name="charts_account_id" id="charts_account_id" class="form-control" required>
                @foreach ($charts_account_petty_cashs as $charts_of_account)
                    <option value="{{ $charts_of_account->id }}" {{ ( $charts_of_account->id == $object->charts_account_id) ? 'selected' : '' }}> {{ $charts_of_account->code }}:{{ $charts_of_account->account_name }} </option>
                @endforeach
        ...
        <div class="form-group">
            ...
            <input type="number" step=".01" class="form-control" id="balance" name="balance"
                   value="{{ $object->balance ?? '' }}" placeholder="Balance" required>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" step=".01" class="form-control" id="refill_amount" name="refill_amount"
                   value="{{ $object->refill_amount ?? '' }}" placeholder="Refill Amount" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            @else
                ...
                <input type="hidden" name="link" value="petty_cash_refill_requests/{{$document_id}}/5">
                <button type="submit" class="btn btn-alt-primary col" name="addItem" value="PettyCashRefillRequest">Submit</button>
            @endif
        </div>
    </form>
</div>
