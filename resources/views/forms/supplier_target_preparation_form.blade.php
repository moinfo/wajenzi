...
<script>
    ...
    $(document).ready(function() {
        ...
        const displayInput = $('<input>', {
            type: 'text',
            class: 'form-control formatted-display',
            readonly: true
        });

        // Insert the display input after the original
        amountInput.after(displayInput);
        amountInput.hide();

        // Format initial value if exists
        if (amountInput.val()) {
            displayInput.val(formatNumber(amountInput.val()));
        }

        // Handle click on formatted display
        displayInput.on('click', function() {
            $(this).hide();
            amountInput.show().focus();
        });

        // Handle input on original amount field
        amountInput.on('input', function() {
            const value = unformatNumber($(this).val());
            $(this).val(value);
            displayInput.val(formatNumber(value));
        });
        ...
        $('form').on('submit', function() {
            // Ensure the original input has the unformatted value
            amountInput.val(unformatNumber(amountInput.val()));
            return true;
        });
    });

    // Style the formatted display to match the original input
    $('<style>')
        .text(`
        .formatted-display {
